...
def center_pitch(experiment, limits, npoints, sampleposition):
    ...
    if limits[0] < limits[1]:
        scan("pitchgi", limits[0], limits[1], npoints, 1,
             experiment, sampleposition)
    else:
        scan("pitchgi", limits[1], limits[0], npoints, 1,
             experiment, sampleposition)
    data = pd.read_csv("current_scan.csv", delimiter = ",",
                       )
...
def zheavy_center(experiment, limits, npoints, sampleposition):
...
def horizontal_center(experiment, limits, npoints, sampleposition):
...
def roll_align(experiment, y_center, sigma_beam, rolloffset, centerofrotation = 30,
    ...
    oldroll = scanmanager.get_pos("rollgi")
